...
bool lbgpsdo_open_device_auto(lbgpsdo_device_t *lbgpsdo_device)
{
  ...
  udev_list_entry_foreach(dev_list_entry, devices)
  {
    ...
    if(
      0 == strcmp( &STR(LBGPSDO_VID_LB_USB)[2], udev_device_get_sysattr_value(devusb,"idVendor"))
      && (
    ...
    )
    {
      ...
      lbgpsdo_device->vid = atoi(udev_device_get_sysattr_value(devusb,"idVendor"));
      lbgpsdo_device->serial = atoi(udev_device_get_sysattr_value(devusb, "serial"));
...
bool lbgpsdo_set_config(lbgpsdo_device_t *lbgpsdo_device, lbgpsdo_config_t *lbgpsdo_config)
{
  ...
  */
  ...
  memcpy(&buf[2], &lbgpsdo_config->GPSFrequency,  sizeof(uint16_t));
